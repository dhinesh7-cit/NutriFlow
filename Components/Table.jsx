export default ({ setCreateShipmentModel, allShipmentsdata }) => {
  ...
  const formatEstimatedDelivery = (pickupTimeInSeconds) => {
    ...
    if (isNaN(pickupTimeInSeconds) || pickupTimeInSeconds <= 0) {
      return "Invalid Pickup Date"; // Or handle appropriately
    }
  
    const pickupDate = new Date(pickupTimeInSeconds * 1000); // Convert seconds to milliseconds
    // Add 7 days (in milliseconds) to the pickup time
    const estimatedDeliveryDate = new Date(pickupDate.getTime() + 7 * 24 * 60 * 60 * 1000);
    ...
    <div className="max-w-screen-xl mx-auto px-4 md:px-8">
      <div className="items-start justify-between md:flex">
        <div className="max-w-lg">
          ...
          View all your current shipments or initiate a new one to be tracked on the blockchain.
          </p>
        </div>
